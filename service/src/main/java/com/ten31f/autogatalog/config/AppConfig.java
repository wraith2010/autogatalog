...
public class AppConfig {
	...
	@Bean
	public Scan scan(WatchURLRepo watchURLRepo, IGatRepo gatRepo) {
	...
	public Downloadrequestor downloadrequestor(TrackingScheduledExecutorService trackingScheduledExecutorService,
			IGatRepo gatRepo, FileRepository fileRepository, LbryRepository lbryRepository) {
		return new Downloadrequestor(lbryRepository, fileRepository, gatRepo, 20, trackingScheduledExecutorService);
	}

	@Bean
	public ImageGrabber imageGrabber(IGatRepo gatRepo, FileRepository fileRepository) {
		return new ImageGrabber(gatRepo, fileRepository, 25);
	}

	@Bean
	public HealthCheck healthCheck(FileRepository fileRepository, IGatRepo gatRepo, HealthRepo healthRepo) {
		return new HealthCheck(fileRepository, gatRepo, healthRepo);
	}

	@Bean
	public PendingDownloadTask pendingDownloadTask(IGatRepo gatRepo) {
